...
const links = [
  ...
  { to: "/about", label: "About us", search: undefined },
  { to: "/payment-policy", label: "Payment policy", search: undefined },
  { to: "/shipping-terms", label: "Shipping terms", search: undefined },
] as const;
...
export function ShopShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/95 backdrop-blur">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <Sheet><SheetTrigger asChild><Button variant="ghost" size="icon" className="md:hidden" aria-label="Open menu"><Menu /></Button></SheetTrigger>
            <SheetContent className="bg-background"><SheetTitle className="font-display text-2xl">Menu</SheetTitle><nav className="mt-10 flex flex-col gap-5">{links.map((link) => <Link key={link.to} to={link.to} search={link.search} className="text-lg font-semibold">{link.label}</Link>)}</nav></SheetContent>
          </Sheet>
        </div>
      </div>
      ...
      <nav className="hidden h-12 items-center justify-center gap-10 border-t border-border/60 md:flex">{links.map((link) => <Link key={link.to} to={link.to} search={link.search} className="text-sm font-bold uppercase tracking-wide text-foreground/80 transition-colors hover:text-primary" activeProps={{ className: "text-primary" }}>{link.label}</Link>)}</nav>
    </header>
    <main>{children}</main>
