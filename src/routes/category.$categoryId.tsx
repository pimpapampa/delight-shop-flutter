...
function CategoryPage() {
  ...
  return <div className="mx-auto max-w-7xl px-5 py-12 sm:px-6 sm:py-16">
    <Link to="/shop" className="inline-flex items-center gap-2 text-sm font-bold text-primary"><ArrowLeft className="size-4" /> All products</Link>
    <div className="mt-8 flex items-center gap-5"><div className={`grid size-24 shrink-0 place-items-center rounded-full ${category.tone}`}><span className="text-5xl" aria-hidden="true">{category.icon}</span></div><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">Shop category</p><h1 className="mt-1 font-display text-4xl sm:text-5xl">{category.name}</h1></div></div>
